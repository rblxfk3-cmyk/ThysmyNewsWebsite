...
| Pembantu BM berpandukan snapshot dan sumber | Pro | Gemini pilihan; panduan automatik jika key tiada/API gagal |
...
4. Tetapkan LIVE_DATA_ENABLED=true dan ENABLE_RECORDER=true. Pembantu automatik terus aktif tanpa konfigurasi AI.
...
- Tiada `OPENAI_API_KEY` atau `OPENAI_MODEL` diperlukan. ENV OpenAI lama diabaikan. Gemini hanya dipanggil jika GEMINI_API_KEY dipasang. Kos hosting/provider lain bergantung pada servis sedia ada.
...
24 ujian deterministik meliputi nombor/unit/revisi, bias per phase, makro tanpa data masa hadapan, kos, bar hilang, immutable snapshots, pemilikan jurnal, freshness/backoff, routes/assets, CSRF, malformed JSON, simulasi/alerts/journal/guide, jumlah bayaran, idempotence Python, pembantu tanpa panggilan model berbayar walaupun ENV lama wujud, status bayaran milik sendiri dan kegagalan schema.
...
## Sambungan Gemini
Upload fail termasuk gemini_ai.py, workspace_api.py, static/features.js dan templates/dashboard.html.
Di Render Environment, tambah GEMINI_API_KEY dan GEMINI_MODEL=gemini-2.5-flash-lite, kemudian Save and deploy.
Kekalkan projek Google pada Free tier; kod tidak boleh menentukan atau mengawal billing akaun Google.
Tiada pertukaran automatik kepada provider/model berbayar. Kuota habis atau API gagal menggunakan panduan aturan.
Buka Tanya THYSMY dan hantar soalan untuk menguji sambungan. Status dikonfigurasi bukan bukti key sah;
status sambungan berjaya hanya selepas respons Gemini diterima. Status reset apabila server restart.
Jika nampak UI lama, refresh halaman selepas deploy siap. Tidak perlu migrasi SQL tambahan untuk Gemini.
Soalan dan snapshot pasaran dihantar kepada Google; jangan taip maklumat sulit. Tiada data jurnal/akaun dihantar automatik.
Rujukan: https://ai.google.dev/gemini-api/docs/pricing dan https://ai.google.dev/api/generate-content
