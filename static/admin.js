...
async function load(){
 ...
 $('system-status').innerHTML=[['Penyimpanan',s.storage.ready?s.storage.backend+(s.storage.durable?' · persistent':' · sementara'):'Perlu kemas kini Supabase V4'],['Pembantu',s.ai?.message||'Panduan automatik'],['Pembayaran',s.payment_configured?s.payment_mode:'Belum dikonfigurasi'],['Sesi',s.session_configured?'Dikonfigurasi':'SESSION_SECRET belum ditetapkan'],['Recorder terakhir',date(s.recorder_last_seen)],['Recorder',s.recorder_error||'Tiada ralat dilaporkan']].map(([k,v])=>'<div class="detail-row"><span>'+esc(k)+'</span><span>'+esc(v)+'</span></div>').join('');
