...
from gemini_ai import GeminiGuide
...
class Workspace:
    def __init__(self,ctx):
        self.ctx=ctx;self.feeds=Feeds();self.store=Store(ctx);self.ai=GeminiGuide()
    ...
    def dashboard(self,request):
        ...
        return {**snap,'ok':True,'version':VERSION,'next_event':{'label':label_for(future),'time':future[0]['time'],'events':future} if future else None,
          ...
          'ai_ready':self.ai.state=='connected','ai':self.ai.status()}
...
def register_workspace(app,ctx):
    ...
    async def assistant(request:Request):
        ...
        ai_result=await run_in_threadpool(w.ai.ask,question,snap)
        if ai_result['answer']:
            return {'answer':ai_result['answer'],'mode':'ai','ai':w.ai.status(),'as_of':snap['server_time'],'sources':sources,'note':'Jawapan Gemini berdasarkan snapshot website; boleh tersilap.'}
        ...
        elif any(t in q for t in ('bias','analisis','buy','sell','sebelum','selepas','actual')) and a:
            answer=f"{a['label']} · {a['phase']} · {a['bias']}\n"+'\n'.join(a['reasons'])
        elif any(t in q for t in ('harga','dxy','us10y')):
            symbol='DXY' if 'dxy' in q else 'US10Y' if 'us10y' in q else 'XAUUSD'
            quote=snap['markets'].get(symbol,{})
            answer=f"{symbol}: {quote.get('price') if quote.get('price') is not None else 'belum tersedia'}. Status sumber: {quote.get('status','unavailable')}. Waktu sumber: {quote.get('timestamp') or 'tiada'}. US10Y ialah data harian; quote tertunda bukan harga live."
            sources.append({'name':symbol,'url':quote.get('source_url','/methodology')})
        elif any(t in q for t in ('nfp','pengangguran','wages')):
            answer='NFP, kadar pengangguran dan pertumbuhan gaji perlu dibaca bersama. Bandingkan actual dengan forecast dan semak revisi. Komponen yang bercanggah boleh menyebabkan reaksi dua hala; lihat Analisis news untuk angka yang tersedia.'
        elif any(t in q for t in ('fomc','fed','faedah')):
            answer='FOMC merangkumi keputusan kadar dan komunikasi dasar. Model angka hanya menilai keputusan yang mempunyai actual dan forecast; nada ucapan dan sidang media tidak dirumus sebagai kepastian arah gold.'
        elif any(t in q for t in ('bercampur','revisi')):
            answer='Data bercampur bermaksud komponen memberi sokongan arah berbeza. Revisi mengubah angka tempoh terdahulu. Lihat komponen dan previous asal dalam Analisis news; satu angka sahaja tidak mencukupi.'
        else:answer='Panduan automatik ini menyokong topik CPI, NFP, FOMC, bias semasa, harga gold/DXY/US10Y, skor volatiliti dan kos trade. Ia bukan model AI generatif. Cuba salah satu topik tersebut; jika data tiada, semak Kalendar.'
        if any(t in q for t in ('bias','analisis','buy','sell','sebelum','selepas','actual')) and a and not any(t in q for t in ('cpi','core','spike','skor','89','volatil','komisen','kos','spread')):
            answer+='\nStatus kalendar: '+str(snap['calendar'].get('status','unavailable'))+' · Dikemas kini: '+str(snap['calendar'].get('retrieved_at') or 'tiada')

        return {'answer':(ai_result['message']+'\n\n' if w.ai.key else '')+answer,'ai':w.ai.status(),'mode':'guide','as_of':snap['server_time'],'sources':sources,'note':'Panduan automatik berasaskan aturan dan data website. Digunakan apabila Gemini tidak dikonfigurasi atau tidak tersedia.'}
    ...
    def admin_system(request:Request):
        ...
        return {'version':VERSION,'storage':w.store.health(),'ai_ready':w.ai.state=='connected','ai':w.ai.status(),
