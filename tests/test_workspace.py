...
def client(tmp_path,monkeypatch):
    ...
    module.workspace.ai=GeminiGuide()
...
def test_assistant_never_calls_paid_model(client,monkeypatch):
    c,m=client;login(client)
    monkeypatch.setenv('OPENAI_API_KEY','unused-test-value')
    monkeypatch.setenv('OPENAI_MODEL','unused-test-model')
    def forbidden(*args,**kwargs):pytest.fail('Guide must not call any model API')
    monkeypatch.setattr(requests,'post',forbidden)
    assert c.get('/api/workspace').json()['ai_ready'] is False
    for question in ('Apa maksud CPI?','Harga gold sekarang?','Terangkan NFP','Apa itu FOMC?','Tulis cerita kucing'):
        r=c.post('/api/assistant',json={'question':question},headers={'X-THYSMY-Request':'1'})
        assert r.status_code==200 and r.json()['mode']=='guide'
    assert 'bukan model AI generatif' in r.json()['answer']


def test_gemini_success_private_context_and_status(client,monkeypatch):
    from gemini_ai import GeminiGuide
    c,m=client;login(client)
    monkeypatch.setenv('GEMINI_API_KEY','private-test-key')
    m.workspace.ai=GeminiGuide()
    sent=[]
    def post(url,**kwargs):
        sent.append(kwargs['json'])
        assert 'private-test-key' not in url
        assert kwargs['headers']['x-goog-api-key']=='private-test-key'
        return SimpleNamespace(status_code=200,json=lambda:{'candidates':[{'finishReason':'STOP','content':{'parts':[{'text':'Helo! Saya boleh bantu terangkan news.'}]}}]})
    monkeypatch.setattr(requests,'post',post)
    assert c.get('/api/workspace').json()['ai_ready'] is False
    r=c.post('/api/assistant',json={'question':'helo'},headers={'X-THYSMY-Request':'1'})
    assert r.status_code==200 and r.json()['mode']=='ai'
    assert r.json()['ai']['state']=='connected'
    import json
    prompt=json.loads(sent[0]['contents'][0]['parts'][0]['text'])
    assert set(prompt['snapshot'])=={'server_time','markets','before','after','calendar'}
    assert 'private-test-key' not in json.dumps(sent)
    assert 'demo@thysmy.local' not in json.dumps(sent)
    assert c.get('/api/workspace').json()['ai_ready'] is True


@pytest.mark.parametrize('status,state',[(429,'quota'),(403,'config_error'),(404,'model_error'),(503,'unavailable')])
def test_gemini_failure_falls_back_and_cools_down(client,monkeypatch,status,state):
    from gemini_ai import GeminiGuide
    c,m=client;login(client)
    monkeypatch.setenv('GEMINI_API_KEY','private-test-key');m.workspace.ai=GeminiGuide()
    calls=[]
    ...
    monkeypatch.setattr(requests,'post',post)
    for _ in range(2):
        r=c.post('/api/assistant',json={'question':'Apa maksud CPI?'},headers={'X-THYSMY-Request':'1'})
        assert r.status_code==200 and r.json()['mode']=='guide'
        assert r.json()['ai']['state']==state
        assert 'CPI mengukur' in r.json()['answer']
        assert 'private-test-key' not in r.text
    assert len(calls)==1


@pytest.mark.parametrize('result',[None,{}, {'candidates':[{'finishReason':'MAX_TOKENS','content':{'parts':[{'text':'unfinished'}]}}]}])
def test_gemini_timeout_and_incomplete_output(monkeypatch,result):
    from gemini_ai import GeminiGuide
    monkeypatch.setenv('GEMINI_API_KEY','private-test-key')
    def post(*args,**kwargs):
        if result is None:raise requests.Timeout('private-test-key')
        return SimpleNamespace(status_code=200,json=lambda:result)
    monkeypatch.setattr(requests,'post',post)
    r=GeminiGuide().ask('hello',{})
    assert r['answer'] is None and 'private-test-key' not in str(r)
